...
for x in lib_types:
    if x.startswith('user '):
        libpaths.append(x)
for x in lib_types:
    if x.strip()=='metabokit':
        libpaths.append('ann_All.txt')
...
def read_lib(libpath):
    ...
    if 'LipidCreatorValidStudy_MRM_Workklist_V1_forHyungWon.txt' in libpath0:
        ...
        for nnn,line in enumerate(qq):
            ...
            frag_I=['1']
            if frag_mz:
                lib_dict[(ms1mz,charge,tuple(frag_mz),tuple(frag_I),adduct,'NA','')].append(name)
                nnnn+=1
        ...
        for line in it_cpd:
            ...
            if lsp[0]=="Name":
                ...
                for lsp in (line.split() for line in it_cpd):
                    ...
                    frag_I.append(lsp[1])
                lib_dict[(ms1mz,charge,tuple(frag_mz),tuple(frag_I),adduct,'NA',formu)].append(name)
        ...
        for line in it_cpd:
            ...
            if lsp[0]=="Name":
                ...
                if (ispos and posneg!='-')or(not ispos and posneg=='-'):
                    lib_dict[(ms1mz,charge,tuple(frag_mz),tuple(frag_I),adduct,'NA',formu)].append(name)
    if 'MSDIAL-TandemMassSpectralAtlas-' in libpath0 or 'MSMS-Public-' in libpath0 or libpath0.startswith('user '):
        lib=' (E)'
        ...
        for line in it_cpd:
            ...
            if lsp[0]=='NAME':
                ...
                for lsp in (line.split() for line in it_cpd):
                    ...
                    frag_I.append(lsp[1])
                if frag_mz:
                    lib_dict[(ms1mz,charge,tuple(frag_mz),tuple(frag_I),adduct,RT,formu)].append(name)
    print(libpath0)
    print(len(lib_dict))
    return lib_dict
...
def get_cpds():#from libs
    ...
    for ent,name in lib_dict.items():
        ...
        lib_ent.append(Ent(ms1mz,name,frag_mz,frag_I,''.join(adduct),charge,RT,formu))
        continue
